<script lang="ts">
	...
	const controlDevices = $derived(
		mergedDevices.filter(
			(device) => device.type !== 'observing_conditions' && device.type !== 'safety_monitor'
		)
	);
	const switchDevices = $derived(controlDevices.filter((device) => device.type === 'switch'));
	const nonSwitchControlDevices = $derived(
		controlDevices.filter((device) => device.type !== 'switch')
	);
	const controlTabs = $derived.by(() => {
		const tabs: Array<{
			id: string;
			label: string;
		...
		if (nonSwitchControlDevices.length > 0) {
			const pageCount = Math.ceil(nonSwitchControlDevices.length / DEVICE_CONTROLS_PER_PAGE);
			...
			for (let index = 0; index < pageCount; index += 1) {
				tabs.push({
					...
					devices: nonSwitchControlDevices.slice(
						index * DEVICE_CONTROLS_PER_PAGE,
						(index + 1) * DEVICE_CONTROLS_PER_PAGE
					)
				});
			}
		}
